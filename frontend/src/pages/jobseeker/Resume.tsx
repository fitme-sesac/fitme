...
type ResumeDataWithAi = ResumeData & { aiSummary?: string };

const initialResumeData: ResumeDataWithAi = {
    personalInfo: {
        name: "",
        email: "",
        phone: "",
...
function mapApiResumeToResumeData(res: {
    ...
    const skills = Array.isArray(reStack)
        ? reStack
        : typeof reStack === "string"
            ? reStack
                .split(",")
                .map((s) => s.trim())
                .filter(Boolean)
    ...
    return {
        ...
        // UI에서 "자기소개/내용"으로 쓰는 값은 content 우선
...
function buildResumeRequest(resumeData: ResumeDataWithAi) {
    ...
    const aiSummary =
        typeof resumeData.aiSummary === "string" && resumeData.aiSummary.trim()
    ...
    return {
        title,
        field: "RESUME",
        // content = 사용자가 편집/저장하는 자기소개(본문)
        content: resumeData.summary ?? "",
        // summary = AI 요약(있으면 유지), 없으면 기존 동작과 동일하게 본문 사용
        summary: aiSummary ?? (resumeData.summary ?? ""),
        tagline: "",
        primary: false,
...
const ResumePage = () => {
    ...
    const [resumeData, setResumeData] = useState<ResumeDataWithAi>(initialResumeData);
    const [activeTab, setActiveTab] = useState("list");
    const [editingResumeId, setEditingResumeId] = useState<number | null>(null);

    const [viewingResumeId, setViewingResumeId] = useState<number | null>(null);
    const [viewingResumeData, setViewingResumeData] = useState<ResumeDataWithAi | null>(null);
    const [loadingViewing, setLoadingViewing] = useState(false);

    // AI 첨삭용 이력서 선택 모달 (현재 코드 기능 유지)
    ...
    }, [resumeData, editingResumeId, updateMutation, createMutation, toast, queryClient]);
    ...
    const handleViewAiSummary = async (id: number) => {
        try {
            ...
            setAiSummaryText(data.aiSummary ?? data.summary ?? "");
                        ...
                        <div className="max-w-2xl w-full">
                            <Card className="overflow-hidden border-none shadow-lg">
                                <div className="bg-gradient-to-br from-sky-500/10 to-sky-500/5 p-10 text-center border-b border-sky-100/50">
                                    ...
                                    <p className="text-gray-500">로그인하고 나만의 전문적인 이력서를 작성해보세요</p>
                                </div>
                                <CardContent className="p-8">
                                    ...
                                    <div className="flex flex-col gap-4">
                                        <Button
                                            asChild
                                            className="w-full text-white h-12 shadow-md border-0 transition-all hover:scale-[1.02]"
        ...
        <>
                        ...
                        <div className="max-w-7xl mx-auto">
                            ...
                            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
                                <div>
                                    ...
                                    <p className="text-gray-500 mt-1">나만의 전문적인 이력서를 작성하고 AI의 도움을 받아보세요</p>
                                </div>
                                <Button
                            ...
                            <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
                                <TabsList className="bg-white p-1 rounded-xl shadow-sm border-none w-full max-w-md grid grid-cols-3">
                                    <TabsTrigger
                                        value="list"
                                        className="gap-2 rounded-lg data-[state=active]:bg-sky-50 data-[state=active]:text-sky-600"
                                    >
                                ...
                                <TabsContent value="list" className="mt-6">
                                    {!listLoading && list.length > 1 && (
                                        <div className="mb-4 p-4 bg-sky-50/50 border border-sky-100 rounded-xl">
                                            <p className="text-sm text-sky-700">
                                                ...
                                                <strong>대표 이력서</strong>를 설정하면 지원 시 기본으로 선택됩니다. 별표 버튼을 클릭하여 대표
                                                이력서를 지정하세요.
                                            </p>
                                        </div>
                                    )}
                                    ...
                                    <div className="grid gap-4">
                                        {listLoading ? (
                                            ...
                                            sortedList.map((resume: { id: number; title?: string; lastModifiedAt?: string; primary?: boolean }) => (
                                                <Card
                                                    key={resume.id}
                                                    className={`border-none shadow-sm hover:shadow-md transition-all bg-white overflow-hidden group ${
                                                        resume.primary ? "ring-2 ring-sky-500/50" : ""
                                                    }`}
                                                >
                                                    <CardContent className="flex items-center justify-between p-6">
                                                        ...
                                                        >
                                                            <div
                                                                className={`h-14 w-14 rounded-xl flex items-center justify-center transition-colors shrink-0 ${
                                                                    resume.primary ? "bg-sky-100" : "bg-sky-50 group-hover:bg-sky-100"
                                                                }`}
                                                            >
                                                            ...
                                                            <div className="min-w-0">
                                                                ...
                                                                <p className="text-sm text-gray-400 mt-1">마지막 수정: {formatResumeDate(resume.lastModifiedAt)}</p>
                                                            </div>
                                                        </div>
                                                        ...
                                                        <div className="flex items-center gap-2 shrink-0" onClick={(e) => e.stopPropagation()}>
                                                            ...
                                                            <Button
                                                                ...
                                                                className={`transition-colors ${
                                                                    resume.primary ? "text-yellow-500 hover:text-yellow-600" : "text-gray-300 hover:text-yellow-500"
                                                                }`}
                                                                onClick={() => {
                                                            ...
                                                            <DropdownMenu>
                                                                ...
                                                                <DropdownMenuContent align="end" className="w-48 p-2">
                                                                    ...
                                                                    <DropdownMenuItem
                                                                        onClick={() => setPrimaryConfirmId(resume.id)}
                                        ...
                                        {!listLoading && list.length === 0 && (
                                            <Card className="border-none shadow-sm bg-white border-dashed border-2 border-gray-100">
                                                <CardContent className="flex flex-col items-center justify-center py-20">
                                                    ...
                                                    <h3 className="text-xl font-bold text-gray-900 mb-2">아직 작성된 이력서가 없습니다</h3>
                                                    <p className="text-gray-500 mb-8">첫 이력서를 작성하고 나만의 경쟁력을 높여보세요!</p>
                                                    <Button
                                                        onClick={handleCreateNew}
                                                        className="text-white border-0 transition-all hover:scale-[1.02]"
                                                        style={{ background: "linear-gradient(90deg, #5AB2FA 0%, #3DCEC9 100%)" }}
                                                    >
                                ...
                                <TabsContent value="edit" className="mt-6">
                                    <div className="space-y-8">
                                        <Card className="border-none shadow-sm bg-white p-6">
                                            ...
                                            <ResumeTemplateSelector selectedTemplate={selectedTemplate} onSelectTemplate={setSelectedTemplate} />
                                        </Card>
                                                ...
                                                <CardContent className="p-8">
                                                    {resumeData.summary ? (
                                                        <div className="prose prose-sky max-w-none">
                                                            ...
                                                            <p className="text-gray-700 whitespace-pre-wrap leading-relaxed">{resumeData.summary}</p>
                ...
                <DialogContent className="sm:max-w-[640px] max-h-[90vh] overflow-y-auto">
                    ...
                    {loadingViewing ? (
                        ...
                        <div className="prose prose-sky max-w-none py-2">
                            ...
                            {!viewingResumeData.summary &&
                            !viewingResumeData.experiences?.length &&
                            !viewingResumeData.skills?.length ? (
            ...
            <Dialog open={resumeSelectModalOpen} onOpenChange={setResumeSelectModalOpen}>
                <DialogContent className="sm:max-w-[480px] max-h-[80vh] overflow-y-auto">
                    ...
                    <div className="py-4">
                        <p className="text-sm text-gray-500 mb-4">AI 첨삭을 받을 이력서를 선택해주세요.</p>
                                ...
                                sortedList.map((resume: { id: number; title?: string; lastModifiedAt?: string; primary?: boolean }) => (
                                    <div
                                        ...
                                        className={`p-4 rounded-xl border-2 cursor-pointer transition-all hover:border-sky-300 hover:bg-sky-50/50 ${
                                            selectedResumeForAI === resume.id ? "border-sky-500 bg-sky-50" : "border-gray-100 bg-white"
                                        }`}
                                        ...
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <div className="flex items-center gap-2">
                                                    <h4 className="font-semibold text-gray-900 truncate">{resume.title ?? "제목 없음"}</h4>
                                                    {resume.primary && (
                                                        <Badge className="bg-sky-500 text-white border-none text-[10px] px-2 py-0 shrink-0">
                                                            대표
                                                        </Badge>
                                                ...
                                                <p className="text-xs text-gray-400 mt-0.5">마지막 수정: {formatResumeDate(resume.lastModifiedAt)}</p>
                                            </div>
                                        </div>
                                    </div>
                                ))
