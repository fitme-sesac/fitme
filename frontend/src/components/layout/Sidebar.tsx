import { Home, Briefcase, FileText, User, Settings, LogIn, LogOut, Sparkles, HelpCircle, Bot, Users, Building2, CreditCard, MessageSquare } from "lucide-react";
    ...
    <aside className="fixed left-0 top-0 z-40 h-screen w-64 border-r border-sidebar-border bg-sidebar hidden lg:flex flex-col">
      ...
      <div className="flex h-16 items-center gap-2 border-b border-sidebar-border px-6">
        <Link to="/" className="flex items-center gap-2">
          <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-[#4E56C0] to-[#9B5DE0] shadow-sm">
            <Sparkles className="h-5 w-5 text-white" />
          </div>
          <span className="text-xl font-bold text-foreground tracking-tight">FitMe</span>
        </Link>
      </div>
      ...
      <div className="border-t border-sidebar-border px-3 py-4">
        ...
        {user ? (
          ...
          <Link
            to="/auth"
            className="mt-3 w-full bg-gradient-to-r from-[#4E56C0] to-[#9B5DE0] text-white shadow-md hover:shadow-lg hover:to-[#D78FEE] transition-all duration-300 flex items-center justify-center gap-2 rounded-xl px-4 py-3 font-bold"
          >
