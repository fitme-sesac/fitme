...
import { employerChatbotQuery } from "@/api/employer-chatbot";
...
interface EmployerAIChatWidgetProps {
    isOpen: boolean;
    onClose: () => void;
    /** 라우트 변경 등 외부 트리거로 채팅 세션을 초기화할 때 사용 */
    resetSeq: number;
    /** 기업 ID (선택, 인증된 사용자의 employer_id) */
    employerId?: number;
}
...
const WELCOME_MESSAGE: Message = {
    ...
    role: "assistant",
    content:
        "안녕하세요! FitMe AI 채용 관리 비서입니다.\n지원자 현황, 공고 성과, 통계 등을 물어보세요!",
    timestamp: new Date(),
};

export function EmployerAIChatWidget({
    isOpen,
    onClose,
    resetSeq,
    employerId,
    ...
    const handleSend = async () => {
        ...
        try {
            const data = await employerChatbotQuery({
                message: text,
            ...
            <Card className="w-[360px] md:w-[400px] h-[600px] shadow-2xl border-border/50 flex flex-col overflow-hidden">
                ...
                >
                    <div className="flex items-center gap-3">
                        <div className="relative">
                            ...
                            <span className="absolute bottom-0 right-0 w-3 h-3 bg-green-400 border-2 border-violet-500 rounded-full"></span>
                        </div>
                            ...
                            >
                                {msg.role === "assistant" && (
                                    <Avatar className="h-8 w-8 mt-1 border">
                                        <AvatarFallback className="bg-violet-50 text-violet-500">
                ...
                <div className="p-4 bg-background border-t shrink-0">
                    <div className="relative flex items-end gap-2">
                        <Input
                            placeholder="지원자 현황, 공고 성과 등을 물어보세요..."
                            value={input}
