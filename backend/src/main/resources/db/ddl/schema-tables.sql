-- ================================================================
-- FitMe PostgreSQL ?ÑÏ≤¥ ?§ÌÇ§Îß?
-- ?§Î•∏ PC?êÏÑú Î∞îÎ°ú ?§Ìñâ Í∞Ä??(IF NOT EXISTS ?ÅÏö©)
-- DB: fitme_project / User: postgres
-- ÏµúÏ¢Ö ?òÏ†ï: 2026-02-03
-- ================================================================
-- ?§Ìñâ Î∞©Î≤ï:
...
-- ================================================================
-- PART 2: ?åÏù¥Î∏??ùÏÑ± (?òÏ°¥???úÏÑú)
-- ================================================================

-- 1) notice (member FK Ï∞∏Ï°∞?©ÏúºÎ°?Î®ºÏ? ?ùÏÑ±)
...
-- 45) talent_proposal (Í∏∞ÏóÖ?íÍµ¨ÏßÅÏûê ?úÏïà)
...
-- 46) industry_vocab (Materialized View - fuzzy Í≤Ä?âÏö©)
...
-- ?ΩÍ?/Í∞úÏù∏?ïÎ≥¥ ?ôÏùò??notice (member Í∞Ä?????ÑÏàò Ï∞∏Ï°∞)
INSERT INTO notice (notice_id, title, body, is_public, notice_type, status)
VALUES 
    (1, '?úÎπÑ???¥Ïö©?ΩÍ?', '?úÎπÑ???¥Ïö©?ΩÍ? ?¥Ïö©?ÖÎãà??', TRUE, 'TERMS', 'ACTIVE'),
    (2, 'Í∞úÏù∏?ïÎ≥¥ Ï≤òÎ¶¨Î∞©Ïπ®', 'Í∞úÏù∏?ïÎ≥¥ Ï≤òÎ¶¨Î∞©Ïπ® ?¥Ïö©?ÖÎãà??', TRUE, 'PRIVACY', 'ACTIVE'),
    (3, '?¥ÏòÅ?ïÏ±Ö', '?¥ÏòÅ?ïÏ±Ö ?¥Ïö©?ÖÎãà??', TRUE, 'POLICY', 'ACTIVE')
...
VALUES
    ('CREDIT_100', 'ONE_TIME', '?¨Î†à??100', 'ON_SALE', 10000.00, 'KRW', 100, NULL),
    ('CREDIT_500', 'ONE_TIME', '?¨Î†à??500', 'ON_SALE', 45000.00, 'KRW', 500, NULL),
    ('CREDIT_1000', 'ONE_TIME', '?¨Î†à??1000', 'ON_SALE', 80000.00, 'KRW', 1000, NULL),
    ('SUB_BASIC', 'SUBSCRIPTION', 'Î≤†Ïù¥Ïß?Íµ¨ÎèÖ', 'ON_SALE', 29000.00, 'KRW', 50, 'BASIC'),
    ('SUB_PRO', 'SUBSCRIPTION', '?ÑÎ°ú Íµ¨ÎèÖ', 'ON_SALE', 99000.00, 'KRW', 200, 'PRO'),
    ('SUB_ENTERPRISE', 'SUBSCRIPTION', '?îÌÑ∞?ÑÎùº?¥Ï¶à Íµ¨ÎèÖ', 'ON_SALE', 299000.00, 'KRW', 1000, 'ENTERPRISE')
...
INSERT INTO notification_template (template_code, channel, subject, body) VALUES
    ('INTERVIEW_SCHEDULED', 'EMAIL', 'Î©¥Ï†ë ?ºÏ†ï???ïÏ†ï?òÏóà?µÎãà??, '{{company_name}}Í≥ºÏùò {{stage}} Î©¥Ï†ë??{{date}}???àÏ†ï?òÏóà?µÎãà??'),
    ('INTERVIEW_REMINDER', 'PUSH', 'Î©¥Ï†ë Î¶¨Îßà?∏Îçî', '{{company_name}} Î©¥Ï†ë??1?úÍ∞Ñ ?ÑÏóê ?úÏûë?©Îãà??'),
    ('APPLICATION_VIEWED', 'PUSH', '?¥Î†•???¥Îûå ?åÎ¶º', '{{company_name}}?êÏÑú ?åÏõê?òÏùò ?¥Î†•?úÎ? ?¥Îûå?àÏäµ?àÎã§.'),
    ('APPLICATION_STATUS', 'EMAIL', 'ÏßÄ???ÑÌô© ?ÖÎç∞?¥Ìä∏', '{{company_name}}??ÏßÄ?êÌïò??{{job_title}} ?¨Ï??òÏùò ?ÅÌÉúÍ∞Ä ?ÖÎç∞?¥Ìä∏?òÏóà?µÎãà??'),
    ('NEW_JOB_MATCH', 'PUSH', '?àÎ°ú??Ï±ÑÏö©Í≥µÍ≥† Ï∂îÏ≤ú', '?åÏõê?òÏùò ?ÑÎ°ú?ÑÍ≥º ÎßûÎäî ?àÎ°ú??Ï±ÑÏö©Í≥µÍ≥†Í∞Ä ?±Î°ù?òÏóà?µÎãà??')
...
SELECT * FROM (VALUES
    ('?åÏõêÍ∞Ä?ÖÏ? ?¥ÎñªÍ≤??òÎÇò??', '?àÌéò?¥Ï? ?∞Ï∏° ?ÅÎã®??"?åÏõêÍ∞Ä?? Î≤ÑÌäº???¥Î¶≠?òÏó¨ ?¥Î©î???êÎäî ?åÏÖú Î°úÍ∑∏?∏ÏúºÎ°?Í∞Ä?ÖÌïò?????àÏäµ?àÎã§.', TRUE, TRUE),
    ('?¥Î†•?úÎäî Î™?Í∞úÍπåÏßÄ ?±Î°ù?????àÎÇò??', 'ÏµúÎ? 5Í∞úÏùò ?¥Î†•?úÎ? ?±Î°ù?òÏã§ ???àÏäµ?àÎã§.', TRUE, TRUE),
    ('Ï±ÑÏö©Í≥µÍ≥† ?±Î°ù ÎπÑÏö©?Ä ?¥ÎñªÍ≤??òÎÇò??', 'Í∏∞Î≥∏ Ï±ÑÏö©Í≥µÍ≥† ?±Î°ù?Ä Î¨¥Î£å?¥Î©∞, ?ÑÎ¶¨ÎØ∏ÏóÑ ?∏Ï∂ú?Ä ?¨Î†à?ßÏùÑ ?¨Ïö©?©Îãà??', TRUE, TRUE),
    ('?¨Î†à?ßÏ? ?¥ÎñªÍ≤?Ï∂©Ï†Ñ?òÎÇò??', 'ÎßàÏù¥?òÏù¥ÏßÄ > ?¨Î†à??Ï∂©Ï†Ñ Î©îÎâ¥?êÏÑú Ï∂©Ï†Ñ?òÏã§ ???àÏäµ?àÎã§.', TRUE, TRUE),
    ('ÏßÄ??Ï∑®ÏÜå???¥ÎñªÍ≤??òÎÇò??', 'ÎßàÏù¥?òÏù¥ÏßÄ > ÏßÄ?êÌòÑ?©Ïóê??"ÏßÄ??Ï∑®ÏÜå" Î≤ÑÌäº???¥Î¶≠?òÏãúÎ©??©Îãà??', TRUE, TRUE)
...
INSERT INTO industry_alias (alias, canonical, display_name) VALUES
    ...
    ('?∏Í≥µÏßÄ??, 'ai', 'AI'),
    ('?∞Ïù¥??, 'data', 'Data'),
    ('?∞Ïù¥?∞ÏÑúÎπÑÏä§', 'data', 'Data'),
    ('mobile', 'mobile', 'Mobile'),
    ('Î™®Î∞î??, 'mobile', 'Mobile'),
    ('fintech', 'fintech', 'Fintech'),
    ('?Ä?åÌÅ¨', 'fintech', 'Fintech'),
    ('ecommerce', 'ecommerce', 'E-Commerce'),
    ('?¥Ïª§Î®∏Ïä§', 'ecommerce', 'E-Commerce'),
    ('Ïª§Î®∏??, 'ecommerce', 'E-Commerce'),
    ('Í≤åÏûÑ', 'game', 'Game'),
    ('game', 'game', 'Game'),
    ('?¨Ïä§ÏºÄ??, 'healthcare', 'Healthcare'),
    ('healthcare', 'healthcare', 'Healthcare'),
    ('edtech', 'edtech', 'EdTech'),
    ('?êÎ??åÌÅ¨', 'edtech', 'EdTech')
...
-- ?ÑÎ£å!
-- ================================================================
-- 1. BATCH_JOB_INSTANCE: Job ?∏Ïä§?¥Ïä§ ?ïÎ≥¥
...
-- 2. BATCH_JOB_EXECUTION: Job ?§Ìñâ ?ïÎ≥¥
...
-- 3. BATCH_JOB_EXECUTION_PARAMS: Job ?§Ìñâ ?åÎùºÎØ∏ÌÑ∞
...
-- 4. BATCH_STEP_EXECUTION: Step ?§Ìñâ ?ïÎ≥¥
...
-- 5. BATCH_STEP_EXECUTION_CONTEXT: Step ?§Ìñâ Ïª®ÌÖç?§Ìä∏
...
-- 6. BATCH_JOB_EXECUTION_CONTEXT: Job ?§Ìñâ Ïª®ÌÖç?§Ìä∏
