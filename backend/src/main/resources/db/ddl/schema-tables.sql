...
--  2) Spring Batch tables (springboot batch ?뚯씠釉?txt) - made idempotent
--  3) Full dummy dataset (fitme_ALL_IN_ONE_13.sql) - session_replication_role set is guarded
--  4) Product seed/upsert (?곹뭹 ?붾??곗씠??txt) - no TRUNCATE, upsert by product_code
--  5) Admin seed/upsert (愿由ъ옄 sql.txt) - upsert by email
--
-- Notes:
...
-- ================================================================
-- FitMe PostgreSQL ?꾩껜 ?ㅽ궎留?-- ?ㅻⅨ PC?먯꽌 諛붾줈 ?ㅽ뻾 媛??(IF NOT EXISTS ?곸슜)
...
-- 1) notice (member FK 李몄“?⑹쑝濡?癒쇱? ?앹꽦)
...
-- 45) talent_proposal (湲곗뾽?믨뎄吏곸옄 ?쒖븞)
...
-- 46) industry_vocab (Materialized View - fuzzy 寃?됱슜)
...
-- ?쎄?/媛쒖씤?뺣낫 ?숈쓽??notice (member 媛?????꾩닔 李몄“)
INSERT INTO notice (notice_id, title, body, is_public, notice_type, status)
VALUES 
    (1, '?쒕퉬???댁슜?쎄?', '?쒕퉬???댁슜?쎄? ?댁슜?낅땲??', TRUE, 'TERMS', 'ACTIVE'),
    (2, '媛쒖씤?뺣낫 泥섎━諛⑹묠', '媛쒖씤?뺣낫 泥섎━諛⑹묠 ?댁슜?낅땲??', TRUE, 'PRIVACY', 'ACTIVE'),
    (3, '?댁쁺?뺤콉', '?댁쁺?뺤콉 ?댁슜?낅땲??', TRUE, 'POLICY', 'ACTIVE')
ON CONFLICT (notice_id) DO UPDATE SET
    title = EXCLUDED.title,
    body = EXCLUDED.body,
    notice_type = EXCLUDED.notice_type;

-- notice_id ?쒗???숆린??SELECT setval(pg_get_serial_sequence('notice', 'notice_id'), GREATEST(COALESCE(MAX(notice_id), 1), 3)) FROM notice;

-- ?곹뭹 ?곗씠??(寃곗젣 ???꾩슂)
INSERT INTO product (product_code, product_type, name, sale_status, price_amount, currency, credit_amount, plan_tier)
VALUES
    ('CREDIT_100', 'ONE_TIME', '?щ젅??100', 'ON_SALE', 10000.00, 'KRW', 100, NULL),
    ('CREDIT_500', 'ONE_TIME', '?щ젅??500', 'ON_SALE', 45000.00, 'KRW', 500, NULL),
    ('CREDIT_1000', 'ONE_TIME', '?щ젅??1000', 'ON_SALE', 80000.00, 'KRW', 1000, NULL),
    ('SUB_BASIC', 'SUBSCRIPTION', '踰좎씠吏?援щ룆', 'ON_SALE', 29000.00, 'KRW', 50, 'BASIC'),
    ('SUB_PRO', 'SUBSCRIPTION', '?꾨줈 援щ룆', 'ON_SALE', 99000.00, 'KRW', 200, 'PRO'),
    ('SUB_ENTERPRISE', 'SUBSCRIPTION', '?뷀꽣?꾨씪?댁쫰 援щ룆', 'ON_SALE', 299000.00, 'KRW', 1000, 'ENTERPRISE')
...
-- ?뚮┝ ?쒗뵆由?INSERT INTO notification_template (template_code, channel, subject, body) VALUES
    ('INTERVIEW_SCHEDULED', 'EMAIL', '硫댁젒 ?쇱젙???뺤젙?섏뿀?듬땲??, '{{company_name}}怨쇱쓽 {{stage}} 硫댁젒??{{date}}???덉젙?섏뿀?듬땲??'),
    ('INTERVIEW_REMINDER', 'PUSH', '硫댁젒 由щ쭏?몃뜑', '{{company_name}} 硫댁젒??1?쒓컙 ?꾩뿉 ?쒖옉?⑸땲??'),
    ('APPLICATION_VIEWED', 'PUSH', '?대젰???대엺 ?뚮┝', '{{company_name}}?먯꽌 ?뚯썝?섏쓽 ?대젰?쒕? ?대엺?덉뒿?덈떎.'),
    ('APPLICATION_STATUS', 'EMAIL', '吏???꾪솴 ?낅뜲?댄듃', '{{company_name}}??吏?먰븯??{{job_title}} ?ъ??섏쓽 ?곹깭媛 ?낅뜲?댄듃?섏뿀?듬땲??'),
    ('NEW_JOB_MATCH', 'PUSH', '?덈줈??梨꾩슜怨듦퀬 異붿쿇', '?뚯썝?섏쓽 ?꾨줈?꾧낵 留욌뒗 ?덈줈??梨꾩슜怨듦퀬媛 ?깅줉?섏뿀?듬땲??')
...
SELECT * FROM (VALUES
    ('?뚯썝媛?낆? ?대뼸寃??섎굹??', '?덊럹?댁? ?곗륫 ?곷떒??"?뚯썝媛?? 踰꾪듉???대┃?섏뿬 ?대찓???먮뒗 ?뚯뀥 濡쒓렇?몄쑝濡?媛?낇븯?????덉뒿?덈떎.', TRUE, TRUE),
    ('?대젰?쒕뒗 紐?媛쒓퉴吏 ?깅줉?????덈굹??', '理쒕? 5媛쒖쓽 ?대젰?쒕? ?깅줉?섏떎 ???덉뒿?덈떎.', TRUE, TRUE),
    ('梨꾩슜怨듦퀬 ?깅줉 鍮꾩슜? ?대뼸寃??섎굹??', '湲곕낯 梨꾩슜怨듦퀬 ?깅줉? 臾대즺?대ŉ, ?꾨━誘몄뾼 ?몄텧? ?щ젅?㏃쓣 ?ъ슜?⑸땲??', TRUE, TRUE),
    ('?щ젅?㏃? ?대뼸寃?異⑹쟾?섎굹??', '留덉씠?섏씠吏 > ?щ젅??異⑹쟾 硫붾돱?먯꽌 異⑹쟾?섏떎 ???덉뒿?덈떎.', TRUE, TRUE),
    ('吏??痍⑥냼???대뼸寃??섎굹??', '留덉씠?섏씠吏 > 吏?먰쁽?⑹뿉??"吏??痍⑥냼" 踰꾪듉???대┃?섏떆硫??⑸땲??', TRUE, TRUE)
...
INSERT INTO industry_alias (alias, canonical, display_name) VALUES
    ...
    ('?댁빱癒몄뒪', 'ecommerce', 'E-Commerce'),
    ('而ㅻ㉧??, 'ecommerce', 'E-Commerce'),
    ('寃뚯엫', 'game', 'Game'),
    ('game', 'game', 'Game'),
    ('?ъ뒪耳??, 'healthcare', 'Healthcare'),
...
-- 1. BATCH_JOB_INSTANCE: Job ?몄뒪?댁뒪 ?뺣낫
...
-- 2. BATCH_JOB_EXECUTION: Job ?ㅽ뻾 ?뺣낫
...
-- 3. BATCH_JOB_EXECUTION_PARAMS: Job ?ㅽ뻾 ?뚮씪誘명꽣
...
-- 4. BATCH_STEP_EXECUTION: Step ?ㅽ뻾 ?뺣낫
...
-- 5. BATCH_STEP_EXECUTION_CONTEXT: Step ?ㅽ뻾 而⑦뀓?ㅽ듃
...
-- 6. BATCH_JOB_EXECUTION_CONTEXT: Job ?ㅽ뻾 而⑦뀓?ㅽ듃
