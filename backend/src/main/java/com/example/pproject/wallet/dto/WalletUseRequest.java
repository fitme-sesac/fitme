...
public record WalletUseRequest(
                @NotNull(message = "사용할 크레딧 양은 필수입니다.") @Min(value = 1, message = "사용 크레딧은 1 이상이어야 합니다.") Long amount,

                @NotNull(message = "주문 ID(멱등키)는 필수입니다.") String orderId,

                @NotNull(message = "사용처(SourceType)는 필수입니다.") SourceType sourceType,
