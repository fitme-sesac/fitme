...
public class ResumeResponse {
    ...
    @Getter @Builder public static class CareerDto {
        ...
        public static CareerDto from(ResumeCareer c) { return CareerDto.builder().id(c.getId()).companyName(c.getCompanyName()).department(c.getDepartment()).role(c.getRole()).startDate(c.getStartDate()).endDate(c.getEndDate()).current(c.getCurrent()).verified(c.getVerified()).build(); }
    }
    ...
    @Getter @Builder public static class CertificateDto {
        ...
        public static CertificateDto from(ResumeCertificate c) { return CertificateDto.builder().id(c.getId()).name(c.getName()).issuer(c.getIssuer()).acquisitionDate(c.getAcquisitionDate()).verified(c.getVerified()).build(); }
    }
    ...
    @Getter @Builder public static class AttachmentDto {
        ...
        public static AttachmentDto from(ResumeAttachment a) { return AttachmentDto.builder().id(a.getId()).fileUrl(a.getFileUrl()).fileName(a.getFileName()).scanStatus(a.getScanStatus()).aiDescription(a.getAiDescription()).build(); }
    }
}