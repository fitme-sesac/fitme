...
public interface AdClickEventRepository extends JpaRepository<AdClickEventEntity, Long> {
        ...
        // [New] 정확한 정산용: 특정 기간 동안의 캠페인별 비용 합계 (SUM(cost))
        @Query("SELECT COALESCE(SUM(e.cost), 0) FROM AdClickEventEntity e " +
                        "WHERE e.campaignId = :campaignId " +
                        "AND e.occurredAt BETWEEN :start AND :end")
        long sumCostByCampaignIdAndOccurredAtBetween(@Param("campaignId") Long campaignId,
