...
public class PasswordResetController {
    ...
    private String externalBaseUrl(HttpServletRequest request) {
        ...
        boolean hostFromForwarded = host != null && !host.isBlank();
        ...
        if ((proto == null || host == null) && forwarded != null && !forwarded.isBlank()) {
            ...
            for (String part : first.split(";")) {
                ...
                String v = p.substring(eq + 1).trim().replace("\"", ""); // ✅ fix

                if ("proto".equals(k) && (proto == null || proto.isBlank())) proto = v;
                if ("host".equals(k) && (host == null || host.isBlank())) {
                    host = v;
                    hostFromForwarded = true;
                }
            }
        }
        ...
        // 포트 결정:
        // - X-Forwarded-Port가 있으면 사용
        // - 없고 forwarded host 기반이면 기본포트(80/443)로 간주
        // - 없고 request 기반이면 request.getServerPort() 사용
        String effectivePort = port;
        if (effectivePort == null || effectivePort.isBlank()) {
            if (hostFromForwarded) {
                effectivePort = "https".equalsIgnoreCase(proto) ? "443" : "80";
            } else {
                effectivePort = String.valueOf(request.getServerPort());
            }
        }
        ...
        // host에 이미 포트가 포함된 경우(예: example.com:443)면 추가하지 않음
        if (!host.contains(":")) {
    ...
    private String publicBackendBase(HttpServletRequest request) {
        ...
        if (!cfg.isBlank()) {
            ...
            if (!cfgIsLocal) return cfg;        // 운영에서 명시 설정
    ...
    private String effectiveFrontBase(HttpServletRequest request) {
        // frontBaseUrl 기본값(localhost:5173)이 운영에서 남아있어도 자동 추론되도록 처리
        ...
        // api.example.com -> example.com 으로 프론트 도메인 추론
        try {
    ...
    private String redirectFront(HttpServletRequest request, String path) {
        return "redirect:" + effectiveFrontBase(request) + path;
    }

    private String redirectFrontWithQuery(HttpServletRequest request, String path, String query) {
        if (query == null || query.isBlank()) return redirectFront(request, path);
        return "redirect:" + effectiveFrontBase(request) + path + "?" + query;
    }

    private Map<String, Object> flowClaimsOf(Claims c) {
    ...
    public String openPasswordResetLink(@RequestParam String token,
        ...
        if (token == null || token.isBlank() || !jwtTokenProvider.validateToken(token)) {
            return redirectFrontWithQuery(request, "/FindPassword", "error=" + enc("링크가 만료되었거나 올바르지 않습니다."));
        }
        ...
        return redirectFrontWithQuery(request, "/NewPassword", "userid=" + enc(userid));
    }
    ...
    public String findPassword(@RequestParam String userid,
        ...
        if (email != null) {
            ...
            return redirectFrontWithQuery(request, "/VerifyCode", "userid=" + enc(userid));
        }

        return redirectFrontWithQuery(request, "/FindPassword", "error=" + enc("정보가 일치하지 않습니다."));
    }

    @PostMapping("/User/Verify_Code")
    public String verifyCode(@RequestParam String inputCode,
                             Model model,
                             HttpServletRequest request, // ✅ fix: request 필요
        ...
        if (tmpToken == null || tmpToken.isBlank() || !jwtTokenProvider.validateToken(tmpToken)) {
            return redirectFrontWithQuery(request, "/FindPassword", "error=" + enc("인증 절차가 만료되었습니다. 다시 시도해주세요."));
        }
        ...
        Map<String, Object> flowClaims = flowClaimsOf(c);
        ...
        return redirectFrontWithQuery(request, "/VerifyCode",
                "userid=" + enc(userid) + "&error=" + enc("인증번호가 일치하지 않습니다."));
    }
    ...
    public String resetPassword(@RequestParam String userid,
        ...
        if (!newPassword.equals(confirmPassword)) {
            return redirectFrontWithQuery(request, "/NewPassword",
                    "userid=" + enc(userid) + "&error=" + enc("비밀번호가 서로 다릅니다."));
        }
        ...
        return redirectFront(request, "/Login");
    }
